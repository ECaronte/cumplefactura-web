    ...
    >
      <div className="container flex flex-col items-center text-center">
        <div className="space-y-6 max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-8 duration-700">
          <h1
            id="hero-title"
            className="text-4xl font-extrabold tracking-tight lg:text-6xl text-primary"
          >
            Factura electrónica fácil para autónomos y negocios con WooCommerce
          </h1>
          <p className="text-xl text-muted-foreground max-w-[800px] mx-auto">
            Cumple la normativa 2026–2027, envía tus registros VeriFactu y
            automatiza la facturación desde tu WooCommerce. Sin fricción. Sin
            líos.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center pt-4">
            ...
            >
              Quiero CumpleFactura{" "}
              <ArrowRight className="ml-2 h-4 w-4" aria-hidden="true" />
            </Button>
            <Button
              size="lg"
              variant="outline"
              className="h-12 px-8 text-lg border-primary text-primary hover:bg-brand-light hover:text-primary"
              asChild
            >
          ...
          <p className="text-sm text-muted-foreground font-medium pt-2">
            Compatible con la factura electrónica obligatoria en España
            (2026–2027).
          </p>
        </div>
      ...
      <div
        className="absolute top-0 z-[-1] h-full w-full bg-white bg-[radial-gradient(ellipse_80%_80%_at_50%_-20%,rgba(59,130,246,0.15),rgba(255,255,255,0))]"
        aria-hidden="true"
      ></div>
    </section>
  );
